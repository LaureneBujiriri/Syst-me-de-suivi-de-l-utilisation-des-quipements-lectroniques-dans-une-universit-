        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-2xl font-bold text-primary mb-6">Modifier la réservation #{{ $booking->id ?? '' }}</h3>
                    ...
                    <form method="POST" action="{{-- route('bookings.update', $booking->id) --}}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="equipment_id" name="equipment_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                {{-- @foreach ($equipments as $equipment) --}}
                                    {{-- <option value="{{ $equipment->id }}" {{ (old('equipment_id') ?? $booking->equipment_id) == $equipment->id ? 'selected' : '' }}>{{ $equipment->name }}</option> --}}
                                    <option value="1" selected>Exemple: Ordinateur Portable Dell</option>
                        ...
                        <div>
                            ...
                            <x-text-input id="start_date" class="block mt-1 w-full" type="date" name="start_date" :value="old('start_date') ?? (isset($booking) ? $booking->start_date->format('Y-m-d') : '')" required />
                        </div>

                        <!-- End Date -->
                        <div>
                            <x-input-label for="end_date" :value="__('Date de fin')" />
                            <x-text-input id="end_date" class="block mt-1 w-full" type="date" name="end_date" :value="old('end_date') ?? (isset($booking) ? $booking->end_date->format('Y-m-d') : '')" required />
                        </div>
